            ...
            with h5py.File(dataset_path, 'r') as root:
                ...
                if compressed:
                    ...
                    for i, cam_name in enumerate(self.camera_names):
                        # print("orin: ",image_dict[cam_name].shape())
                        # 获取真实图像数据长度
                        image_len = len(image_dict[cam_name].nonzero()[0]) # 找到非零部分
                        # image_len =
                        compressed_image = image_dict[cam_name][:image_len]  # 获取有效数据部分
                        decompressed_image = cv2.imdecode(compressed_image, 1)
                        image_dict[cam_name] = np.array(decompressed_image)
                        # print("now: ", image_dict[cam_name].shape())
...
if __name__ == '__main__':
    ...
    batch_size_train = 1
    batch_size_val = 1
    load_data(dataset_dir, True, camera_names, batch_size_train, batch_size_val, (50),
              (True), (False), "ACT", stats_dir_l=None,
              sample_weights=None, train_ratio=0.9)
