...
def main(args):
   ...
   # robot init
    ...
    model = Imitate_Model(ckpt_dir='./ckpt/orange9', ckpt_name=model_name)#carbat_slow carbat344 orange422  153-419  orange416_9
    model.loadModel()
    print("model init success...")

    # Initialize the parameters
    episode_len = 280#400  330 540# The total number of steps to complete the task. Note that it must be less than or equal to parameter 'episode_len' of the corresponding task in file 'ModelTrain.constants'
    t=0
    last_time = 0
    ...
    while t < episode_len:
        ...
        if first:
            max_delta = (np.abs(last_action - action)).max()
            steps = min(int(max_delta / 0.001), 100)#min
            for jnt in np.linspace(last_action, action, steps):
                env.step(jnt,np.array([0,1]))
                # time.sleep(1)
            first = False
        ...
        time3 = time.time()
        print("action: ", action)
        obs = env.step(action,np.array([1,1]))# 控制机械臂
        time4 = time.time()
        ...
        print("Read joint value time(ms)：", (time4 - time3) * 1000,"第几步： ",t)
        t +=1
