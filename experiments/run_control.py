    ...
    while not is_falling[0]:
        ...
        for i in range(2):
            ...
            if dev_keys[i, 0] == 1 and start_press_status[i, 0]:  # button a: end
                ...
                if toc-tic < 0.5:
                    ...
                    if keys_press_count[i, 0] % 2 == 1: #解锁 移动
                        what_to_do[i, 0] = 1
                    ...
                    if keys_press_count[i, 1] % 2 == 1:#跟随
                        what_to_do[i, 1] = 1
        ...
        for i in range(2):
            ...
            if dev_keys[i, 1] == 1:
                ...
                if keys_press_count[0, 2] % 2 == 1:#录制
                    if keys_press_count[0, 1] % 2 == 1 or keys_press_count[1, 1] % 2 == 1:#跟随情况下
                        what_to_do[0, 2] = 1
...
def main(args):
    ...
    cameranames = ['top', 'left_wrist', 'right_wrist']
    ...
    total_time = 0.04
    # img_lists
    recordda = hdf5data(dataset_dir=save_dir,camera_names=cameranames)#"/home/robot/datasets/dataset_package_test/train_data"
    # camerasall = {}
    # pdb.set_trace()
    while 1:
        tic = time.time()
        ...
        toc1 = time.time()
        t_time = toc1-tic
        # print("judge time(ms): ", t_time*1000)
        toc3 = time.time()
        if (what_to_do[0, 1] or what_to_do[1, 1]) and start_servo:#跟随状态
            timeif0 = time.time()
            action = agent.act({})
            err3, action = servo_action_check(action, last_action, flag_in)
            assert err3 != 0, set_light(env, "red", 1)
            timeif2 = time.time()
            # print("if0 time: ", (timeif2 - timeif0) * 1000, "ms")
            # ×××××××××××××××××××××××××××××Security protection×××××××××××××××××××××××××××××××××××××××××××
            ...
            # ×××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××
            timeif1 = time.time()
            # print("if time: ", (timeif1 - timeif0) * 1000, "ms")
            ...
            if what_to_do[0, 2] == 1:#录制
                steps += 1
                print("录制")
                firstepisode = True
                camerasall = {}
                for i in range(3):
                    camerasall[cameranames[i]] = img_list[i].copy()#copy.deepcopy(img_list[i]) #img_list[i]# copy.deepcopy 深拷贝  .copy()

                recordda.update_data(np.array(obs["joint_positions"] ),np.array(obs["joint_velocities"]),np.array(action),camerasall)

            if what_to_do[0, 2] == 0:  # 结束录制
                # 检查文件是否存在
                # hdf5_file = f"{idx}.hdf5"  # 假设文件名是 idx.hdf5
                hdf5_file = os.path.join(save_dir, f"{idx}.hdf5")
                if (not os.path.exists(hdf5_file))  and firstepisode:  # 如果文件不存在，保存数据并递增 idx
                # if firstepisode:
                        print(f"保存数据到 {hdf5_file}")
                        recordda.save_data(hdf5_file)  # 保存数据（假设 save_data 方法会保存到 {idx}.hdf5）
                        idx += 1  # 递增 idx
                        # print("停止录制")
                        firstepisode = False  # 标记为已经保存，防止多次保存
                elif os.path.exists(hdf5_file) and firstepisode:
                    idx += 1
                    print(f"文件 {hdf5_file} 已存在，跳过保存，idx ++")
                else:
                    print("已保存")
            timeimg0 = time.time()
            obs = env.step(action, flag_in)
            timeimg3 = time.time()
            # print("step robot time: ", (timeimg3 - timeimg0) * 1000, "ms")
        ...
        toc2 = time.time()
        t_time1 = toc2-tic
        # print("judge time1 (ms): ", t_time1*1000)
        t_time2 = toc2 - toc3
        # print("judge time1 (ms): ", t_time2 * 1000)
        # img show
        # if args.show_img:
        ...
        #     show_canvas[:, :640] = np.asarray(camerasall["left_wrist"], dtype="uint8")
        #     show_canvas[:, 640:640 * 2] = np.asarray(camerasall["top"], dtype="uint8")
        #     show_canvas[:, 640 * 2:640 * 3] = np.asarray(camerasall["right_wrist"], dtype="uint8")
        #
