...
class DobotRobot(Robot):
    ...
    def __init__(self, robot_ip: str = "192.168.5.1", no_gripper: bool = False, robot_number: int = 1,clear: bool = False):
        ...
        if clear:#clear=True 表示清除报警且下使能
            self.r_inter.ClearError()
            time.sleep(0.1)
            self.r_inter.DisableRobot()
            time.sleep(0.1)
